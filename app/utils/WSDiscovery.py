...
class AddressMonitorThread(_StopableDaemonThread):
    def __init__(self, wsd):
        self._addrs = set()
        print("address monitor thread __init", wsd)
        self._wsd = wsd
    ...
    def _updateAddrs(self):
        ...
        for addr in new:
            print("network ADDress addr", addr)
...
class NetworkingThread(_StopableDaemonThread):
    ...
    def addSourceAddr(self, addr):
        ...
        print("NetworkingThread self._multiOutUniInSockets", self._multiOutUniInSockets)
    ...
    def run(self):
        print(self._quitEvent.is_set(),self._queue)
    ...
    def _recvMessages(self):
        print(self.inputs)
        read_list, write_list, err_list = select.select(self.inputs, self.outputs, self.errors)
        print("read_list", read_list)
        # for item , event in read_list:
    ...
    def start(self):
        ...
        self.inputs = [self._uniOutSocket]
...
class WSDiscovery:
    ...
    def _networkAddressAdded(self, addr):
        print("WSDiscovery _networkAddressAdded addr", addr)
